...
/**
 * Updates core domain component JSON files by encoding CSX rule files to base64
...
function processWorkflowDirectory(dirPath, specificFile = null) {
    ...
    try {
        ...
        if (updatesCount > 0) {
            ...
            console.log(`⚠️  No CSX rules found to update in core component: ${targetFileName}`);
        }
    } catch (error) {
...
if (args.length === 0) {
    // Default to core/Workflows directory (can work with any component directory)
...
console.log('🚀 core Domain Component CSX Updater');
...
try {
    ...
    console.log('✅ core domain component update completed');
} catch (error) {

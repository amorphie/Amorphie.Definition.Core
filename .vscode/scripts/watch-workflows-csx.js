...
/**
 * Watches core domain CSX files and automatically updates component JSON files
...
console.log(`👀 core Domain Component Watcher`);
...
console.log(`📁 Found ${componentDirs.length} component directories to watch in core domain:`);
        ...
        try {
            const watcher = fs.watch(srcDir, { recursive: false }, (eventType, filename) => {
                if (filename && filename.endsWith('.csx')) {
                    console.log(`\n📝 Detected change in core: ${filename} (${eventType})`);
...
process.on('SIGINT', () => {
    console.log('\n\n🛑 Stopping core domain file watchers...');
    ...
    console.log('✅ core domain watchers stopped');
    process.exit(0);
});

// Keep the process alive
console.log(`\n✅ core domain file watchers are active. Modify any .csx file to see automatic updates.`);
console.log(`\n💡 Tip: Use the following tasks in VS Code:`);
console.log(`   - "Update Current core CSX" (current file)`);
console.log(`   - "Update All core CSX" (all components)`);
